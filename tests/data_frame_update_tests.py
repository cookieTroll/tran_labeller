...
import numpy as np
from postprocessor import update_categories, generate_aggregate_data, save_output
...
class TestUpdateCategories:
    ...
    def test_invalid_update_dict_syntax(self, sample_df):
        ...
        with pytest.raises(ValueError, match="At index 0, the result is not a dictionary"):
...
class TestGenerateAggregateData:
    def test_successful_aggregation(self, sample_df):
        ...
        assert all(col in result.columns for col in ['date', ('amount', 'sum'), 
    ...
    def test_successful_save(self, tmp_path, sample_dfs, sample_logs):
        ...
        with pd.ExcelFile(output_file) as xlsx:
            ...
            pd.testing.assert_frame_equal(
                pd.read_excel(xlsx, "Original Data"),
                raw_data
            )
            pd.testing.assert_frame_equal(
                pd.read_excel(xlsx, "Processed Data"),
                parsed_data
            )
            pd.testing.assert_frame_equal(
                pd.read_excel(xlsx, "Updated Categories"),
                final_data
            )
            pd.testing.assert_frame_equal(
                pd.read_excel(xlsx, "Monthly Expenses"),
                aggregate_data
            )
