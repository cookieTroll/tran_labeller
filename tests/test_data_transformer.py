...
def test_basic_date_parsing(sample_formats):
    ...
    input_dates = pd.Series(["01/12/2023 15:30:00", "02/12/2023 09:45:00"])
    expected = pd.Series([datetime(2023, 12, 1, 15, 30), datetime(2023, 12, 2, 9, 45)])
...
def test_null_values(sample_formats):
    ...
    input_dates = pd.Series(["01/12/2023", None, np.nan, "02/12/2023"])

    expected = pd.Series([datetime(2023, 12, 1), pd.NaT, pd.NaT, datetime(2023, 12, 2)])
...
def test_clean_overlapping_fields_no_overlap():
    ...
    df = pd.DataFrame(
        {"col1": ["apple", "banana", "cherry"], "col2": ["orange", "grape", "berry"]}
    )
    result = clean_overlapping_fields(df, ["col1", "col2"])
...
def test_clean_overlapping_fields_with_nan():
    ...
    df = pd.DataFrame(
        {"col1": ["apple", np.nan, "cherry"], "col2": ["big apple", "banana", np.nan]}
    )
    expected = pd.DataFrame(
        {"col1": [np.nan, np.nan, "cherry"], "col2": ["big apple", "banana", np.nan]}
    )
    result = clean_overlapping_fields(df, ["col1", "col2"])
    assert_frame_equal(result, expected, check_dtype=False)
...
def test_clean_overlapping_fields_mixed_types():
    ...
    df = pd.DataFrame(
        {"col1": [123, "text", True], "col2": ["123 more", "some text", "True value"]}
    )
    expected = pd.DataFrame(
        {
            "col1": [np.nan, np.nan, np.nan],
